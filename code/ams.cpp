...
int main(int argn, char **argv)
{
    ...
    uint format_graph=1;  // 0: do not format graph, 1: form the forward graph, 2: form the reverse graph.
    ...
    for (auto k:data)
    {
        ...
        for(usint i=eta_start;i<eta_end;i++)
        {
            ...
            for(auto alg:algs)
            {
                ...
                TAlg Alg(R_graph);
                Alg.set_parameters(model, cost, {}, eta, eps_Inf, delta_Inf, result_dir);
